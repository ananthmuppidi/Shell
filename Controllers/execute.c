...
int checkBackground(char command[])
{
    ...
    if (!strcasecmp(tokenizedCommand[tokens - 1], "&"))
    { // if the last element of the tokenized command is a @
        //        printf("& detected");
        strcpy(reconstructedCommand, tokenizedCommand[0]);
        strcat(reconstructedCommand, " ");
        for (int i = 1; i < tokens - 1; i++)
        { // the first token was already copied and the & must not be copied
...
int execute(char *command)
{
    ...
    else if (!strcasecmp(tokenizedCommand[0], "sig"))
    {
        sig(commandCopy);
    }
    else if (!strcasecmp(tokenizedCommand[0], "fg"))
    {
        fg(commandCopy);
    }
    else if (!strcasecmp(tokenizedCommand[0], "bg"))
    {
        bg(commandCopy);
    }
    else if (!strcasecmp(tokenizedCommand[0], "pinfo"))
    {
        pinfo(commandCopy);
    }
    else
